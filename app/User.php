...
class User extends Authenticatable implements MustVerifyEmail
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'phone', 'password','role_id', 'status','device_token','token_fcm'
    ];
    ...
    {
     return $this->belongsToMany('App\Category','user_category','user_id','category_id')->select('name_en', 'name_ar', 'slug');
    }
    ...
    {
        return $this->hasOne('App\Terms','id','badge_id')->select('id','title_en', 'title_ar')->with('preview');
    }
    ...
    {
        return $this->hasOne('App\Terms','auth_id')->where('type',10)->where('status',1)->select('auth_id','title_en', 'title_ar','count');
    }

    public function shoptag()
    {
     return $this->belongsToMany('App\Category','user_category','user_id','category_id')->where('type',2)->select('name_en','slug','type');
    }

    public function reserve_image()
    {
        return $this->hasMany('App\ReservationImage','restaurant_id','id');
    }
