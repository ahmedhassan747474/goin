...
@section('content')
@include('layouts.backend.partials.headersection',['title'=> __('All Products')])
<div class="row">
				...
				<div class="float-right">
					<form>
						<div class="input-group mb-2 col-12">
							...
							<input type="text" class="form-control" placeholder="{{ __('Search...') }}"  name="src" autocomplete="off" value="{{ $src ?? '' }}">
					...
					<table class="table">
						<thead>
							<tr>
								...
								<th class="am-tags">{{ __('Restaurant') }}</th>
						...
						<tbody>
							...
							<tr>
								...
								<td>@if($post->status==1)  {{ __('Published') }} @elseif($post->status==2)  {{ __('Draft') }} @else {{ __('Trash') }} @endif</td>
								<td>{{ __('Last Modified') }}
									<div class="date">
