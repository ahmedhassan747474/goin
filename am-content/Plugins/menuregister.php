...
function RegisterAdminMenuBar(){
	...
	if(Auth::User()->role->id == 1){
		...
		if (Auth()->user()->can('slider.list')) {
			$data['admin_media']=array(
				'name' => __('Slider'),
				'active' => Request::is('admin/slider'),
				'icon' => 'far fa-images',
		...
		if (Auth()->user()->can('group.list')) {
			if (Auth()->user()->can('group.list')) {
				$groups[__('Group')] = route('admin.group.index');
			}
			// if (Auth()->user()->can('group.product')) {
			// 	$groups[__('Products of Group')] =  route('admin.group.product.index');
			// }

			if (count($groups ?? []) > 0) {
				$data['admin_group']=array(
					'name' => __('Group'),
					'icon' => 'far fa-images',
					'child'=> $groups
				);
		...
		if (Amcoders\Plugin\Plugin::is_active('shop')) {
			...
			if (Auth()->user()->can('resturents.add')) {
				$vendors[__('Add Resturent')] = route('admin.vendor.create');
			}
			...
			if (count($vendors ?? []) > 0) {
				$is_active = Request::is('admin/resturents*') || Request::is('admin/add_resturent');
				$data['vendors']=array(
					'name' => __('Restaurant'),
		...
		if (Auth()->user()->can('reservation.list')) {
			$reservations[__('All Reservations')] = route('admin.reservation.index');
		}
		if (Auth()->user()->can('reservation.control')) {
			$reservations[__('Pending Reservations')] = route('admin.reservation.type','pending');
			$reservations[__('Accept Reservations')] = route('admin.reservation.type','accepted');
			$reservations[__('Decline Reservations')] = route('admin.reservation.type','decline');
			$reservations[__('Make Reservation')] = route('admin.reservation.create');
		}

		if (count($reservations ?? []) > 0) {
			$data['shop_reservation']=array(
				'name' => __('Reservations'),
				'icon' => 'fas fa-cubes',
				'active'=>Request::is('admin/reservation*'),
				'child'=> $reservations
			);
		}
	...
	if(Auth::User()->role->id == 3){
		...
		if (Amcoders\Plugin\Plugin::is_active('shop')) {
			...
			$data['reserve_image']=array(
				'name' => __('Reservation Image'),
				'active' => Request::is('store/reserve_image'),
				'icon' => 'far fa-images',
				'url' => route('store.reserve_image.index')
			);

// 			$data['menu']=array(
// 				'name' => __('Category'),
// 				'icon' => 'fas fa-list-ul',
// 				'active' => Request::is('store/menu'), 
...
// 			);
			...
			// 	'name' => __('Size'),
			// 	'icon' => 'fas fa-list-ul',
			// 	'active' => Request::is('store/size'), 
			// 	'url' => route('store.size.index')
			// );
			...
			$data['reservations']=array(
				'name' => __('Reservations'),
				'icon' => 'fas fa-cubes',
				'active' => Request::is('store/reservations'), 
				'url' => route('store.reservation.index')
			);
			
			$data['tables']=array(
				'name' => __('Tables'),
				'active' => Request::is('store/tables'),
				'icon' => 'far fa-images',
				'url' => route('store.tables.index')
			);
