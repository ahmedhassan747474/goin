				...
				<form id="basicform" method="post" action="{{ route('store.menu.store') }}">
					...
					<div class="custom-form">
						<div class="form-group">
							...
							<input type="text" name="name_en" class="form-control" id="name_en" placeholder="Menu Name English">
						</div>

						<div class="form-group">
							<label for="name">{{ __('Name') }} (Arabic)</label>
							<input type="text" name="name_ar" class="form-control" id="name_ar" placeholder="Menu Name Arabic">
						</div>
	...
	<div class="col-lg-8" >      
		<div class="card">
			<div class="card-body">
				@php
				if (!empty($req)) {
					$categeories=\App\Category::where('type',1)->where('user_id',Auth::id())
					->where('name_en','LIKE','%'.$req.'%' )
					->where('name_ar','LIKE','%'.$req.'%' )
					->latest()->paginate(12);	
				}
				else{
