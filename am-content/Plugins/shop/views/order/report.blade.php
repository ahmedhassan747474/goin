             ...
             <div class="form-group ml-2">
               ...
               <select class="form-control" name="status">
                ...
                @endphp
                 ...
                 <option value="1" {{$st == '1' ? 'selected' : ''}}>{{ __('Completed') }}</option>
               </select>
             </div>
      ...
      <div class="float-right">
        <form>
          <div class="input-group mt-3 col-12">
            ...
            <input type="text" class="form-control" placeholder="{{__('Search By Order ID')}}" required="" name="src" value="{{ $src ?? '' }}">
            ...
            <tr>
              ...
              <td> <div class="dropdown d-inline">
                ...
                <div class="dropdown-menu">
                  <a class="dropdown-item has-icon" href="{{ route('store.order.show',$row->id) }}"><i class="fas fa-eye"></i> {{__('View')}}</a>
                  <a class="dropdown-item has-icon" target="_blank" href="{{ route('store.invoice_print',$row->id) }}"><i class="fas fa-print"></i> {{__('Print Now')}}</a>
                   <a class="dropdown-item has-icon" href="{{ route('store.invoice',$row->id) }}"><i class="fas fa-file-invoice"></i> {{__('Download Invoice')}}</a>
                </div>
              </div>
            </td>
            </tr>
