...
@include('layouts.backend.partials.headersection',['title'=>isset($type) ? ucfirst($type).' Reservations' : 'Reservations List'])
<div class="row">
		...
		<div class="card">
			<div class="card-body">
				...
				<div class="float-left">
					<form action="{{ route('admin.reservation.date.filter') }}" method="GET">
						<div class="d-flex">
				...
				<table class="table table-responsive">
					<thead>
						<tr>
							<th class="am-title">{{ __('Reservation ID') }}</th>
							<th class="am-author">{{ __('Restaurant Name') }}</th>
              <th class="am-tags">{{ __('Name') }}</th>
              <th class="am-tags">{{ __('Email') }}</th>
              <th class="am-tags">{{ __('Phone') }}</th>
              <th class="am-tags">{{ __('Person') }}</th>
              <th class="am-tags">{{ __('Status') }}</th>
              <th class="am-tags">{{ __('Date') }}</th>
              <th class="am-tags">{{ __('Time') }}</th>
              ...
              <td> 
                <div class="dropdown d-inline">
                  ...
                  <div class="dropdown-menu" x-placement="top-start" >
                    <div class="dropdown-title">Reservation No #{{ $order->id }}</div>
                    <a class="dropdown-item" href="{{ route('admin.reservation.details',$order->id) }}">{{ __('View') }}</a>
                    <a class="dropdown-item cancel" href="{{ route('admin.reservation.delete',$order->id) }}">{{ __('Delete') }}</a>
                  </div>
                </div>
            	</td>  
						</tr>
